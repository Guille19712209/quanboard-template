...
// CAMBIOS EN CY desde OBJECT STYLE 
...
// Llamar a la función cuando cargue el modal (opcional)
document.getElementById('m-table-model').addEventListener('show.bs.modal', () => {
  generarTabla(datos);
});

/// Cada vez que se oculta un modal
document.addEventListener('hidden.bs.modal', function () {
  // Buscar todos los elementos de backdrop que quedaron
  document.querySelectorAll('.modal-backdrop').forEach(backdrop => {
    backdrop.remove(); // eliminarlos
  });
  
  // Opcional: sacar la clase 'modal-open' del body si quedó
  document.body.classList.remove('modal-open');
  document.body.style = ''; // limpiar estilos que Bootstrap mete
});