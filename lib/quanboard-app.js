...
document.addEventListener("DOMContentLoaded", () => {
  ...
  while (deshabilitadas.size < 5) {
    deshabilitadas.add(Math.floor(Math.random() * 12));
  }
  ...
  for (let fila = 0; fila < 12; fila++) {
    ...
    th.textContent = `Object ${fila + 1}`;
    th.classList.add("fw-bold", "text-start", "fs-7");
    tr.appendChild(th);
    ...
    for (let col = 0; col < 12; col++) {
      ...
      input.type = "number";
      input.placeholder = Math.floor(Math.random() * (1000 - 5 + 1)) + 5;
