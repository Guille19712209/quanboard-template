...
const datos = [
  Array(12).fill('Object 1'),
  Array(12).fill('Object 2'),
  Array(12).fill('Object 3'),
  Array(12).fill('Object 4'),
  Array(12).fill('Object 5'),
  Array(12).fill('Object 6')
];

function generarTabla(arrayDeDatos) {
...
document.getElementById('m-table-model').addEventListener('show.bs.modal', () => {
  generarTabla(datos);
});
...
// TABLA EJEMPLO
 ...
 const meses = ["Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"];
    const mesesDesdeMayo = [...meses.slice(4), ...meses.slice(0, 4)]; // Desde Mayo
    ...
    // Primera fila del thead (números)
    filaNumeros.innerHTML = `<th rowspan="2">Objeto</th>` +
      mesesDesdeMayo.map((_, i) => `<th>${i + 1}</th>`).join("");

    // Segunda fila del thead (meses)
    filaMeses.innerHTML = mesesDesdeMayo.map(mes => `<th>${mes}</th>`).join("");

    // Generar cuerpo de tabla con datos aleatorios
    nombresObjetos.forEach(nombre => {
      const fila = document.createElement("tr");
      fila.innerHTML = `<th class="text-start fw-bold">${nombre}</th>` +
        mesesDesdeMayo.map(() => `<td>${Math.floor(Math.random() * (1000 - 5 + 1)) + 5}</td>`).join("");
