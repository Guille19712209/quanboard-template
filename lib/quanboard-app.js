const cy = cytoscape({
    ...
    elements: [
      ...
      { data: { id: 'f', title: '', name: 'Object F', nameSize: "1em", value: '500',valueSize: "2em", unit: '$', unitSize: "1em",shape: 'star', nodeColor: "rgba(0,128,255,0.7)",  fontColor: "rgba(100, 100, 100, 1)",size: 150 } },

      { data: { id: '1', source: 'a', target: 'b', line:'solid', curve:'unbundled-bezier' } },
      { data: { id: '2',source: 'c', target: 'b', line:'dashed', curve:'straight'  } },
      { data: { id: '3',source: 'e', target: 'c', line:'solid', curve:'unbundled-bezier'  } },
      { data: { id: '4',source: 'f', target: 'a', line:'dashed', curve:'straight'  } },
      { data: { id: '5',source: 'a', target: 'd', line:'solid', curve:'unbundled-bezier'  } },
      { data: { id: '6',source: 'd', target: 'b', line:'solid', curve:'unbundled-bezier'  } },
      { data: { id: '7',source: 'a', target: 'c', line:'dashed', curve:'straight'  } },
      { data: { id: '8',source: 'c', target: 'b', line:'solid', curve:'unbundled-bezier'  } }
    ],
    style: [
      {
        selector: 'node',
...
document.addEventListener("DOMContentLoaded", () => {
  const headerNumeros = document.getElementById("header-numeros");
  const headerMeses = document.getElementById("header-meses");
  const cuerpo = document.getElementById("tabla-cuerpo");

  const meses = ["jan 25", "feb 25", "mar 25", "apr 25", "may 25", "jun 25",
                  "jul 25", "ago 25", "set 25", "oct 25", "nov 25", "dic 25","jan 26", "feb 26", "mar 26", "apr 26", "may 26", "jun 26"];

  for (let i = 1; i <= 18; i++) {
    const thNum = document.createElement("th");
    thNum.textContent = i;
    headerNumeros.appendChild(thNum);
    headerNumeros.classList.add("fw-light", "text-center", "fs-3");

    const thMes = document.createElement("th");
    thMes.textContent = meses[i - 1];
    headerMeses.appendChild(thMes);
    headerMeses.classList.add("fw-light", "text-center", "fs-7");
  }

  const deshabilitadas = new Set();
  while (deshabilitadas.size < 5) {
    deshabilitadas.add(Math.floor(Math.random() * 18));
  }
  ...
  for (let fila = 0; fila < 18; fila++) {
    ...
    th.textContent = `Parent.Parent.Object ${fila + 1}`;
    th.classList.add( "text-start", "fs-6","px-2");
    tr.appendChild(th);

    const th1 = document.createElement("th");
    th1.textContent = `$`;
    th1.classList.add("fw-light", "text-center", "fs-6");
    tr.appendChild(th1);

    const isDisabled = deshabilitadas.has(fila);
    ...
    for (let col = 0; col < 18; col++) {
      ...
      input.type = "number";
      input.placeholder = (Math.floor(Math.random() * (1000 - 5 + 1)) + 5).toFixed(2);
      input.classList.add("form-control", "text-center","cell-width");
